...
public class Keyboard extends GLFWKeyCallback{
	...
	public void invoke(long window, int key, int scancode, int action, int mods) {
        SetKey(key, action != GLFW_RELEASE);
	}

    private void SetKey(int key, boolean pressed){
        if(pressed && !KeyStates.contains(key))
            KeyStates.add(key);

        if(!pressed && KeyStates.contains(key))
            KeyStates.remove((Integer)key);
    }

	public static boolean isClicked(int key){
		return KeyStates_ThisFrame.contains(key) && !KeyStates_LastFrame.contains(key);
	}
	
	public static boolean isPressed(int key){
		return KeyStates_ThisFrame.contains(key);
	}
    ...
    public static int GetKeyState(Display display, int key){
        return glfwGetKey(display.getWindowID(), key);
    }

    public static void BeginFrame(){
        KeyStates_ThisFrame.clear();
        KeyStates_ThisFrame.addAll(KeyStates);
    }
